...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"status":"success","message":"If the email exists, you'll receive OTP"}, status=status.HTTP_200_OK)
        otp_obj = create_email_otp(email, purpose='password_reset')
        send_otp_email(email, otp_obj.otp, purpose='password_reset')
        return Response({"status":"success","message":"OTP sent if email exists"}, status=status.HTTP_200_OK)
class ResetPasswordConfirmView(APIView):
    ...
    def post(self, request):
        ...
        obj = EmailOTP.objects.filter(email=email, purpose='password_reset').order_by('-created_at').first()
        if not obj or obj.is_expired() or obj.otp != otp:
            return Response({"status":"error","message":"Invalid/expired OTP"}, status=status.HTTP_400_BAD_REQUEST)
        user = User.objects.get(email=email)
