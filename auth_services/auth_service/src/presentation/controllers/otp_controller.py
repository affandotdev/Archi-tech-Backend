...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        obj = EmailOTP.objects.filter(email=email, purpose=purpose).order_by('-created_at').first()
        if not obj:
            return Response({"status":"error","message":"OTP not found"}, status=status.HTTP_400_BAD_REQUEST)
        if obj.is_expired():
