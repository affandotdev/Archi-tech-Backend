    ...
    def post(self, request):
        ...
        try:
            ...
            if not requester_profile.role:
                print(
                    f"DEBUG: Profile exists but role is empty. Defaulting to 'client'."
                )
            ...
            print(
                f"DEBUG: Profile missing for user {request.user.id}. Creating default Client profile."
            )
        ...
        obj, created = ConnectionRequest.objects.get_or_create(
            requester_id=str(request.user.id), target_id=str(target_id)
        )
        print(
            f"DEBUG: ConnectionRequest created? {created}. Obj ID: {obj.id}, Status: {obj.status}"
        )
...
class PendingRequests(APIView):
    ...
    def get(self, request):
        ...
        qs = ConnectionRequest.objects.filter(
            target_id=user_id, status=ConnectionRequest.STATUS_PENDING
        )
...
class ApproveRequest(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"detail": "ok", "status": req.status}, status=status.HTTP_200_OK
        )
...
class CheckAccess(APIView):
    ...
    def get(self, request, target_id):
        ...
        allowed = (
            ConnectionRequest.objects.filter(status=ConnectionRequest.STATUS_APPROVED)
            .filter(
...
class ConnectedUsers(APIView):
    ...
    def get(self, request):
        ...
        received_ids = ConnectionRequest.objects.filter(
            target_id=user_id, status=ConnectionRequest.STATUS_APPROVED
        ...
        sent_ids = ConnectionRequest.objects.filter(
            requester_id=user_id, status=ConnectionRequest.STATUS_APPROVED
...
class RemoveConnection(APIView):
    ...
    def post(self, request):
        ...
        if not connection:
            return Response(
                {"detail": "Connection not found"}, status=status.HTTP_404_NOT_FOUND
            )
