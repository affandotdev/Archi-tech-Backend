    ...
    if not firebase_admin._apps:
        ...
        if firebase_creds_json:
            ...
            if os.path.exists(cred_path):
                ...
                print(
                    f"[FCM] WARNING: Credentials not found (Env 'FIREBASE_CREDENTIALS_JSON' or File '{cred_path}')."
                )
...
def send_push_notification(user_id: str, title: str, body: str):
    ...
    tokens_query = FCMToken.objects.filter(user_id=user_id).values_list(
        "token", flat=True
    )
    ...
    try:
        ...
        print(
            f"[FCM] Sent to {user_id}: {response.success_count} success, {response.failure_count} failed"
        )
