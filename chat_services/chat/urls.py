...
from .views import (ConversationCreateView, ConversationListView,
                    MessageCreateView, MessageListView, NotificationListView,
                    NotificationMarkReadView, RegisterFCMTokenView)

urlpatterns = [
    path(
        "conversations/", ConversationCreateView.as_view(), name="conversation-create"
    ),
    ...
    path(
        "notifications/<int:notification_id>/read/", NotificationMarkReadView.as_view()
    ),
]
